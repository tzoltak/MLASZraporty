...
subtitle: |
  | `r params$nazwa_szkoly`
  | `r params$adres_szkoly`
output:
  pdf_document:
    dev: cairo_pdf
    fig_caption: false
...
#                 plikZObiektami = "C:/Users/Bartek/Documents/01_IBE/00_pakiety/MLASZraporty/data/wskazniki_do_szablonu_dane2rm.RData",
#                 obiektWskazniki = "szkola",
#                 obiektWskaznikiPorownanie = "wojSzk",
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
tab_num = function() {
  nr = get0("___nrTabeli___", .GlobalEnv, ifnotfound = 1)
  assign("___nrTabeli___", nr + 1, envir = .GlobalEnv)
  return(nr)
}
wyk_num = function() {
  nr = get0("___nrWykresu___", .GlobalEnv, ifnotfound = 1)
  assign("___nrWykresu___", nr + 1, envir = .GlobalEnv)
  return(nr)
}
head_2 = function() {
  nr = get0("___nrRozdz2___", .GlobalEnv, ifnotfound = 1)
  assign("___nrRozdz2___", nr + 1, envir = .GlobalEnv)
  return(nr)
}
head_3 = function() {
  nr = get0("___nrRozdz3___", .GlobalEnv, ifnotfound = 1)
  assign("___nrRozdz3___", nr + 1, envir = .GlobalEnv)
  return(nr)
}
```
...
if (szkola$ocena_pnz[[1]]$n >= progLiczebnosci) {
  ...
  wykres = ggplot(tab, aes(x = reorder(typ, desc(typ)), y = freq, fill = rozklad)) +
    ...
    "**Wykres ",
    wyk_num(),
    ": Opinie uczniów o tym, czy co najmniej jedna forma praktycznej nauki zawodu przygotowała ich dobrze lub bardzo dobrze do wykonywania wyuczonego zawodu**\n\n",
    sep = ""
  ))
  print(wykres)
}
...
if (szkola$przyg_zaw_zaj_ANY[[1]]$n >= progLiczebnosci) {
  ...
  wykres = ggplot(tab, aes(x = reorder(typ, desc(typ)), y = freq, fill = rozklad)) +
    ...
    "**Wykres ",
    wyk_num(),
    ": Opinie uczniów o tym na ile zajęcia praktyczne przygotowały ich do wykonywania zawodu, którego uczyli się w tej szkole**\n\n",
    sep = ""
  ))
  print(wykres)
}
